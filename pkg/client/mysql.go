...
func InitMySQL() (db *gorm.DB, err error) {
	...
	if err != nil {
		return nil, errno.NewErrNo(errno.InternalDatabaseErrorCode, "dal.InitMySQL get mysql DSN error")
	}
	...
	if err != nil {
		return nil, errno.NewErrNo(errno.InternalDatabaseErrorCode, "dal.InitMySQL mysql connect error")
	}

	sqlDB, err := db.DB() // 尝试获取 DB 实例对象
	if err != nil {
		return nil, errno.NewErrNo(errno.InternalDatabaseErrorCode, "get generic database object error")
	}
	...
	if err = sqlDB.Ping(); err != nil {
		return nil, errno.NewErrNo(errno.InternalDatabaseErrorCode, "ping database error")
	}
